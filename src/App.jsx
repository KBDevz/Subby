...
const ROLE_OPTIONS = ["GK","DEF","MID","FWD"];
const PLAYER_STATUSES = {
  available: { label:"Available", short:"Avail", color:"#4ade80" },
  tired: { label:"Tired", short:"Tired", color:"#f59e0b" },
  resting: { label:"Resting", short:"Rest", color:"#a78bfa" },
  injured: { label:"Injured", short:"Inj", color:"#ef4444" },
};
...
const makeDefaultPlayerPrefs = () => Array.from({length:15},()=>({ preferredRole:"", keeper:false }));
const normalizePlayerPrefs = prefs => Array.from({length:15},(_,i)=>({
  preferredRole: prefs?.[i]?.preferredRole || "",
  keeper: !!prefs?.[i]?.keeper,
}));
const extractPlayerPrefs = customForms => customForms?._playerPrefs ? normalizePlayerPrefs(customForms._playerPrefs) : null;
const embedPlayerPrefs = (customForms, playerPrefs) => ({ ...(customForms || {}), _playerPrefs: normalizePlayerPrefs(playerPrefs) });
...
async function fireAlert() {
  ...
  // Best-effort browser notification when permission is granted
  try {
    if ("Notification" in window && Notification.permission === "granted") {
      const n = new Notification("Subby: Time to Sub", {
        body: "Your next substitution window is ready.",
        tag: "subby-sub-alert",
        renotify: true,
  ...
  return (
    <div style={{position:"relative",width:"100%",paddingBottom:"120%",borderRadius:16,overflow:"hidden",boxShadow:"0 8px 32px rgba(0,0,0,0.6)"}}>
      ...
      {positions.map((pos,pi)=>{
        ...
        const safeX=Math.min(84,Math.max(16,pos.x));
        const safeY=Math.min(108,Math.max(16,pos.y));
        const topPct=(safeY/120)*100;
        return (
          <div key={pi} onClick={()=>onTapPos(pi)}
            style={{position:"absolute",left:`${safeX}%`,top:`${topPct}%`,transform:"translate(-50%,-50%)",display:"flex",flexDirection:"column",alignItems:"center",cursor:"pointer",zIndex:10,userSelect:"none"}}>
            <div style={{
...
// ─── PickerSheet ─────────────────────────────────────────────
function PickerSheet({ posIdx, positions, assigned, roster, numbers, notes, getTotal, getStint, onAssign, onClear, onClose, phase, getPlayerStatus, setPlayerStatus }) {
  ...
  assigned.forEach((playerId, idx)=>{
    if(playerId!=null) playerAssignments.set(playerId, idx);
  });
  const allPlayers=[...roster].sort((a,b)=>{
    const aAssigned = playerAssignments.has(a.id) ? 1 : 0;
    const bAssigned = playerAssignments.has(b.id) ? 1 : 0;
    if(aAssigned!==bAssigned) return aAssigned-bAssigned;
    return a.name.localeCompare(b.name);
  });
  const avail = phase==="game"
    ? allPlayers.filter(p=>!playerAssignments.has(p.id))
    : allPlayers;
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.72)",zIndex:300,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:"#161b22",borderRadius:"22px 22px 0 0",padding:"20px 20px 44px",width:"100%",maxWidth:480,border:"1px solid #30363d",animation:"sheetUp .22s ease"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:10}}>{phase==="game"?"Substitute In":"Assign or Swap"}</div>
        <div style={{fontSize:11,color:"#6b7280",marginBottom:10,lineHeight:1.4}}>
          {phase==="game"
            ? "Only bench players are shown here for quick subs."
            : "Tap anyone on the field to swap them here instantly."
          }
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:8}}>
          {avail.map(p=>{
            const assignedIdx = playerAssignments.get(p.id);
            const assignedPos = assignedIdx!=null ? positions[assignedIdx] : null;
            const isCurrent = p.id===curId;
            return (
            <button key={p.id} onClick={()=>onAssign(posIdx,p.id)}
              style={{background:isCurrent?"#1f3a2a":"#21262d",border:`1px solid ${isCurrent?"#4ade80":assignedPos?"rgba(255,255,255,0.14)":"#30363d"}`,borderRadius:12,padding:"12px 12px",display:"flex",flexDirection:"column",justifyContent:"space-between",alignItems:"flex-start",color:"white",cursor:"pointer",textAlign:"left",minHeight:88}}>
              <div style={{width:"100%"}}>
                <div style={{fontWeight:700,fontSize:14,lineHeight:1.2,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>
                  {p.name}
                </div>
                {numbers[p.id]&&<div style={{fontSize:11,color:"#8b949e",marginTop:4}}>#{numbers[p.id]}</div>}
                {assignedPos&&(
                  <div style={{display:"inline-flex",alignItems:"center",gap:4,marginTop:6,background:`${ROLE_COLOR[assignedPos.role]}18`,border:`1px solid ${ROLE_COLOR[assignedPos.role]}44`,borderRadius:999,padding:"2px 7px",fontSize:10,color:ROLE_COLOR[assignedPos.role],fontWeight:700}}>
                    {assignedPos.role} on field
                  </div>
                )}
                {notes[p.id]&&<div style={{fontSize:10,color:"#6b7280",marginTop:4,lineHeight:1.35,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{notes[p.id]}</div>}
              </div>
              {phase==="game"&&(
                <div style={{textAlign:"left",flexShrink:0,marginTop:8}}>
          ...
          {avail.length===0&&<div style={{gridColumn:"1 / -1",color:"#8b949e",textAlign:"center",padding:20,fontSize:13}}>All players assigned</div>}
        </div>
          ...
          <div key={entry.f} onClick={()=>onSelect(entry.f)}
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                {isCustom&&<button onClick={e=>{e.stopPropagation();onDelete(entry.f);}} style={{background:"#21262d",border:"1px solid #30363d",color:"#6b7280",borderRadius:6,width:22,height:22,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",lineHeight:1,flexShrink:0}}>×</button>}
              </div>
            </div>
          </div>
        );
...
function SeasonView({ onClose }) {
  ...
  const [seasonTab,setSeasonTab]=useState("games");
  const seasonPlayerTotals = season.reduce((acc, game, gameIndex)=>{
    const playersById = new Map((game.playerStats || []).map(p=>[p.id, p]));
    (game.playerStats || []).forEach(player=>{
      const entry = acc.get(player.id) || {
        id: player.id,
        name: player.name,
      ...
      acc.set(player.id, entry);
    });
    (game.goalEvents || []).forEach(event=>{
      if(event.type!=="goal") return;
      const scorer = playersById.get(event.scorerId);
      if(scorer){
        const entry = acc.get(scorer.id) || { id: scorer.id, name: scorer.name, number: scorer.number, minutes: 0, goals: 0, assists: 0, games: 0, appearances: [] };
        entry.goals += 1;
        const gameAppearance = entry.appearances.find(appearance=>appearance.gameIndex===gameIndex);
        if(gameAppearance) gameAppearance.goals += 1;
        acc.set(scorer.id, entry);
      }
      if(event.assistId!=null){
        const assister = playersById.get(event.assistId);
        if(assister){
          const entry = acc.get(assister.id) || { id: assister.id, name: assister.name, number: assister.number, minutes: 0, goals: 0, assists: 0, games: 0, appearances: [] };
          entry.assists += 1;
          const gameAppearance = entry.appearances.find(appearance=>appearance.gameIndex===gameIndex);
          if(gameAppearance) gameAppearance.assists += 1;
          acc.set(assister.id, entry);
        }
      }
    });
    return acc;
  }, new Map());
  const seasonPlayers = [...seasonPlayerTotals.values()].sort((a,b)=>{
    if(b.goals!==a.goals) return b.goals-a.goals;
    if(b.assists!==a.assists) return b.assists-a.assists;
    if(b.minutes!==a.minutes) return b.minutes-a.minutes;
    return a.name.localeCompare(b.name);
  });
  const seasonAttackLeaders = [...seasonPlayers].sort((a,b)=>{
    if(b.goals!==a.goals) return b.goals-a.goals;
    if(b.assists!==a.assists) return b.assists-a.assists;
    return a.name.localeCompare(b.name);
  });
  const matchupLabel = game => {
    if (!game?.opponent) return game?.teamName || "Game";
  ...
  const shareGame=(game)=>{
    ...
    const text=[`${game.teamName} ${game.score?.team ?? 0}-${game.score?.opponent ?? 0} ${game.opponent || "Opponent"}`, `${new Date(game.date).toLocaleDateString()} · ${matchupLabel(game)}`, `Formation: ${game.formation}`,"",
  ...
  if (playerSel!==null) {
    const player = seasonPlayers.find(p=>p.id===playerSel);
    if(player) {
      const perGame = [...player.appearances].sort((a,b)=>new Date(b.date)-new Date(a.date));
      return (
        <div style={{padding:"16px 16px 48px",animation:"pageIn .25s ease"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
            <div>
              <button onClick={()=>setPlayerSel(null)} style={{background:"transparent",border:"none",color:"#8b949e",fontSize:12,cursor:"pointer",padding:0,marginBottom:4}}>← All Players</button>
              <div style={{fontSize:18,fontWeight:800}}>{player.name}{player.number&&<span style={{color:"#8b949e",fontSize:12}}> #{player.number}</span>}</div>
              <div style={{fontSize:12,color:"#8b949e",marginTop:4}}>Season production and appearances</div>
            </div>
          </div>

          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
            {[["Minutes",fmtMin(player.minutes),"#4ade80"],["Goals",String(player.goals),"#4ade80"],["Assists",String(player.assists),"#60a5fa"]].map(([label,value,color])=>(
              <div key={label} style={{background:"#161b22",border:"1px solid #21262d",borderRadius:12,padding:"12px 10px"}}>
                <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:6}}>{label}</div>
                <div style={{fontSize:20,fontWeight:800,color}}>{value}</div>
              </div>
            ))}
          </div>

          <div style={{background:"#161b22",border:"1px solid #21262d",borderRadius:12,padding:"12px 14px",marginBottom:12}}>
            <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:8}}>Games Played</div>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:13}}>
              <span style={{color:"#e6edf3",fontWeight:700}}>{player.games} appearances</span>
              <span style={{color:"#8b949e"}}>{player.games ? fmtMin(Math.round(player.minutes / player.games)) : "0s"} avg</span>
            </div>
          </div>

          <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:8}}>Per Game</div>
          <div style={{display:"grid",gap:8}}>
            {perGame.map((appearance, idx)=>(
              <div key={`${appearance.gameIndex}-${idx}`} style={{background:"#161b22",border:"1px solid #21262d",borderRadius:12,padding:"12px 14px"}}>
                <div style={{display:"flex",justifyContent:"space-between",gap:10,marginBottom:6}}>
                  <div>
                    <div style={{fontSize:13,fontWeight:700}}>{new Date(appearance.date).toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"})}</div>
                    <div style={{fontSize:11,color:"#8b949e",marginTop:2}}>{appearance.homeAway==="away"?"@":"vs"} {appearance.opponent || "Opponent"} · {appearance.formation}</div>
                  </div>
                  <div style={{fontSize:12,color:"#e6edf3",fontWeight:700,whiteSpace:"nowrap"}}>{appearance.score?.team ?? 0} - {appearance.score?.opponent ?? 0}</div>
                </div>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  <span style={{background:"rgba(74,222,128,0.12)",border:"1px solid rgba(74,222,128,0.35)",borderRadius:999,padding:"2px 8px",fontSize:10,color:"#4ade80",fontWeight:800}}>
                    {fmtMin(appearance.total)}
                  </span>
                  {appearance.goals>0&&<span style={{background:"rgba(74,222,128,0.12)",border:"1px solid rgba(74,222,128,0.35)",borderRadius:999,padding:"2px 8px",fontSize:10,color:"#4ade80",fontWeight:800}}>{appearance.goals} G</span>}
                  {appearance.assists>0&&<span style={{background:"rgba(96,165,250,0.12)",border:"1px solid rgba(96,165,250,0.35)",borderRadius:999,padding:"2px 8px",fontSize:10,color:"#60a5fa",fontWeight:800}}>{appearance.assists} A</span>}
                </div>
              </div>
            ))}
          </div>
  ...
  if (sel!==null) {
    ...
    const gameGoalsByPlayer = new Map();
    const gameAssistsByPlayer = new Map();
    (game.goalEvents || []).forEach(event=>{
      if(event.type!=="goal") return;
      gameGoalsByPlayer.set(event.scorerId, (gameGoalsByPlayer.get(event.scorerId) || 0) + 1);
      if(event.assistId!=null) gameAssistsByPlayer.set(event.assistId, (gameAssistsByPlayer.get(event.assistId) || 0) + 1);
    });
    return (
      <div style={{padding:"16px 16px 48px",animation:"pageIn .25s ease"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div>
            ...
            <div style={{fontSize:11,color:"#8b949e"}}>{matchupLabel(game)} · {game.formation}</div>
            <div style={{fontSize:11,color:"#4b5563",marginTop:2}}>{game.teamName}</div>
          </div>
        ...
        <div style={{background:"#161b22",borderRadius:12,padding:"14px 16px",border:"1px solid #21262d",display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
          <div>
            <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:6}}>Final Score</div>
            <div style={{fontSize:15,fontWeight:700}}>{game.teamName}</div>
            <div style={{fontSize:12,color:"#8b949e",marginTop:2}}>{game.opponent || "Opponent"}</div>
          </div>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:46,letterSpacing:2,lineHeight:0.9,color:"#4ade80"}}>
            {game.score?.team ?? 0}<span style={{color:"#6b7280",margin:"0 6px"}}>-</span><span style={{color:"#e6edf3"}}>{game.score?.opponent ?? 0}</span>
          </div>
        </div>
        {game.goalEvents?.length>0&&(
          <div style={{background:"#161b22",borderRadius:12,padding:"12px 14px",border:"1px solid #21262d",marginBottom:12}}>
            <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:8}}>Goals</div>
            <div style={{display:"grid",gap:8}}>
              {game.goalEvents.map((event,i)=>{
                const scorer = game.playerStats.find(p=>p.id===event.scorerId);
                const assister = game.playerStats.find(p=>p.id===event.assistId);
                return (
                  <div key={event.id || i} style={{display:"flex",justifyContent:"space-between",gap:10,fontSize:12}}>
                    <div>
                      {event.type==="goal"
                        ? <span><strong style={{color:"#4ade80"}}>{scorer?.name || "Unknown"}</strong>{assister ? <span style={{color:"#8b949e"}}> · Assist {assister.name}</span> : null}</span>
                        : <span style={{color:"#f87171",fontWeight:700}}>Opponent goal</span>
                      }
                    </div>
                    <div style={{color:"#8b949e",whiteSpace:"nowrap"}}>{fmt(event.time || 0)}</div>
                  </div>
                );
              })}
            </div>
        ...
        {sorted.map(p=>{
          ...
          return (
            <div key={p.id} style={{background:"#161b22",borderRadius:12,padding:"12px 14px",border:"1px solid #21262d",marginBottom:8}}>
              ...
              {(gameGoalsByPlayer.get(p.id) || gameAssistsByPlayer.get(p.id)) ? (
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:6}}>
                  {(gameGoalsByPlayer.get(p.id) || 0) > 0 && (
                    <span style={{background:"rgba(74,222,128,0.12)",border:"1px solid rgba(74,222,128,0.35)",borderRadius:999,padding:"2px 8px",fontSize:10,color:"#4ade80",fontWeight:800}}>
                      {gameGoalsByPlayer.get(p.id)} Goal{gameGoalsByPlayer.get(p.id) > 1 ? "s" : ""}
                    </span>
                  )}
                  {(gameAssistsByPlayer.get(p.id) || 0) > 0 && (
                    <span style={{background:"rgba(96,165,250,0.12)",border:"1px solid rgba(96,165,250,0.35)",borderRadius:999,padding:"2px 8px",fontSize:10,color:"#60a5fa",fontWeight:800}}>
                      {gameAssistsByPlayer.get(p.id)} Assist{gameAssistsByPlayer.get(p.id) > 1 ? "s" : ""}
                    </span>
                  )}
                </div>
              ) : null}
        ...
        <div style={{display:"grid",gap:10}}>
          <div style={{display:"flex",gap:8}}>
            {[
              ...
              <button key={value} onClick={()=>setSeasonTab(value)}
                style={{flex:1,padding:"10px 12px",background:seasonTab===value?"#4ade80":"#161b22",border:`1px solid ${seasonTab===value?"#4ade80":"#21262d"}`,borderRadius:10,color:seasonTab===value?"#0d1117":"#8b949e",fontSize:12,fontWeight:800,cursor:"pointer"}}>
                {label}
              </button>
            ))}
          </div>

          {seasonAttackLeaders.length>0&&(
            <div style={{background:"#161b22",borderRadius:12,padding:"14px 16px",border:"1px solid #21262d"}}>
              <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:10}}>Season Leaders</div>
              <div style={{display:"grid",gap:8}}>
                {seasonAttackLeaders.slice(0,3).map(player=>(
                  <div key={player.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
                    <div style={{minWidth:0}}>
                      <div style={{fontSize:13,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                        {player.name}{player.number?<span style={{color:"#8b949e",fontSize:11}}> #{player.number}</span>:null}
                      </div>
                    </div>
                    <div style={{display:"flex",gap:6,flexShrink:0}}>
                      <span style={{background:"rgba(74,222,128,0.12)",border:"1px solid rgba(74,222,128,0.35)",borderRadius:999,padding:"3px 8px",fontSize:10,color:"#4ade80",fontWeight:800}}>
                        {player.goals} G
                      </span>
                      <span style={{background:"rgba(96,165,250,0.12)",border:"1px solid rgba(96,165,250,0.35)",borderRadius:999,padding:"3px 8px",fontSize:10,color:"#60a5fa",fontWeight:800}}>
                        {player.assists} A
                      </span>
                    </div>
                  </div>
          ...
          {seasonTab==="games" ? season.map((game,i)=>{
            ...
            return (
              <div key={i} onClick={()=>setSel(i)}
                ...
                <div>
                  ...
                  <div style={{fontSize:11,color:"#8b949e",marginTop:2}}>{matchupLabel(game)}</div>
                  <div style={{fontSize:10,color:"#4b5563",marginTop:2}}>{game.formation} · {game.playerStats.length} players</div>
                  <div style={{fontSize:12,color:"#e6edf3",marginTop:4,fontWeight:700}}>{game.score?.team ?? 0} - {game.score?.opponent ?? 0}</div>
            ...
            <button key={player.id} onClick={()=>setPlayerSel(player.id)}
              style={{background:"#161b22",borderRadius:12,padding:"14px 16px",border:"1px solid #21262d",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",textAlign:"left",color:"#e6edf3"}}>
              <div style={{minWidth:0}}>
                <div style={{fontSize:14,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                  {player.name}{player.number?<span style={{color:"#8b949e",fontSize:11}}> #{player.number}</span>:null}
                </div>
                <div style={{fontSize:10,color:"#4b5563",marginTop:4}}>{player.games} games · {fmtMin(player.minutes)}</div>
              </div>
              <div style={{display:"flex",gap:6,marginLeft:10,flexShrink:0}}>
                <span style={{background:"rgba(74,222,128,0.12)",border:"1px solid rgba(74,222,128,0.35)",borderRadius:999,padding:"3px 8px",fontSize:10,color:"#4ade80",fontWeight:800}}>{player.goals} G</span>
                <span style={{background:"rgba(96,165,250,0.12)",border:"1px solid rgba(96,165,250,0.35)",borderRadius:999,padding:"3px 8px",fontSize:10,color:"#60a5fa",fontWeight:800}}>{player.assists} A</span>
              </div>
            </button>
          ))}
          {season.length>0&&(
...
// ─── Landing Page ─────────────────────────────────────────────
function LandingPage({ onLogin, sbReady }) {
...
export default function App() {
  ...
  const [playerPrefs,setPlayerPrefs] = useState(()=>normalizePlayerPrefs(lsGet("scm_playerPrefs", makeDefaultPlayerPrefs())));
  ...
  useEffect(()=>{
    if(!user){ setTeamLoaded(false); return; }
    const sb=getSupabase(); if(!sb){ setTeamLoaded(true); return; }
      ...
      sb.from("teams").select("*").eq("invite_code", pendingInvite).single()
        .then(async ({data, error}) => {
          if(data && !error) {
            ...
            if(data.custom_formations) {
              setCF(data.custom_formations);
              const embeddedPrefs = extractPlayerPrefs(data.custom_formations);
              if(embeddedPrefs) setPlayerPrefs(embeddedPrefs);
            }
          ...
          sb.from("teams").select("*").contains("members",[user.id]).single()
            .then(({data:td})=>{
              if(td){
                ...
                if(td.custom_formations) {
                  setCF(td.custom_formations);
                  const embeddedPrefs = extractPlayerPrefs(td.custom_formations);
                  if(embeddedPrefs) setPlayerPrefs(embeddedPrefs);
                }
  ...
  const saveToCloud = useCallback(async(overrides={})=>{
    ...
    const payload={
      ...
      custom_formations: embedPlayerPrefs(overrides.customForms??customForms, overrides.playerPrefs??playerPrefs),
  ...
  },[user,teamName,playerCount,names,numbers,notes,formation,customForms,playerPrefs,subIntervalMins,autoSub]);
  ...
  useEffect(()=>lsSet("scm_playerPrefs",playerPrefs),[playerPrefs]);
  ...
  const [autoSubSuggestions,setAutoSubSuggestions] = useState([]); // [{posIdx,outId,inId,score}]
  ...
  const [showFormationSettings,setShowFormationSettings] = useState(true);
  ...
  const [presentPlayerSince,setPresentPlayerSince] = useState({});
  const [presentPlayersReturnPhase,setPresentPlayersReturnPhase] = useState("field");
  ...
  useEffect(()=>{
    if(!user || !teamLoaded) return;
    const hasSavedTeam = !!teamName.trim() || names.some(n=>n.trim());
    if(!hasSavedTeam) {
      setPhase(prev=>prev==="home" ? "setup" : prev);
    }
  },[user, teamLoaded, teamName, names]);
  ...
  const autoAssignCycleRef = useRef(0);
  ...
  const computeRef  = useRef(null); // set after computeBestSwaps is defined
  ...
  const masterRoster   = names.map((n,i)=>({id:i,name:n.trim()})).filter(p=>p.name);
  const roster         = masterRoster.filter(p=>presentPlayerIds.includes(p.id));
  const seasonGames    = lsGet("scm_season",[]);
  const latestGame     = seasonGames[0] || null;
  const onField        = new Set(assigned.filter(id=>id!=null));
  const bench          = roster.filter(p=>!onField.has(p.id));
  const lineupComplete = assigned.length===positions.length && assigned.every(id=>id!=null);
  ...
  const latestGameLabel = latestGame
    ? latestGame.homeAway==="away"
      ? `@ ${latestGame.opponent || "Opponent"}`
      : `vs ${latestGame.opponent || "Opponent"}`
    : null;
  const opponentLabel = gameOpponent.trim() || "Opponent";
  ...
  // Fairness
  const fairness = id=>{
    const availableSince = presentPlayerSince[id] ?? 0;
    const availableDuration = Math.max(0, gameTime - availableSince);
    if(availableDuration===0 || roster.length===0) return "ok";
    const expectedTime = (availableDuration * playerCount) / roster.length;
    if(expectedTime===0) return "ok";
    const r=getTotal(id)/expectedTime;
    return r<0.55?"low":r<0.78?"warn":"ok";
  };
  const fairColor = {ok:"#4ade80",warn:"#f59e0b",low:"#ef4444"};
  const coachName = (user?.user_metadata?.full_name || user?.email?.split("@")[0] || "Coach").trim();
  const coachInitials = coachName
    ...
    .join("") || "C";
  const updatePlayerPref = (index, patch) => {
    setPlayerPrefs(prev=>{
      const next = normalizePlayerPrefs(prev);
      next[index] = { ...next[index], ...patch };
      return next;
    });
  };
  const getPlayerStatus = id => playerStatuses[id] || "available";
        ...
        setSC(prev=>{
          ...
          if(next===0 && prev>0){
            ...
            if(autoSubRef.current && computeRef.current){
              const swaps=computeRef.current();
              setAutoSubSuggestions(swaps);
            }
          }
          return next;
        });
  ...
  const computeBestSwaps = useCallback(()=>{
    const benchPlayers = roster.filter(p => {
      if(onField.has(p.id)) return false;
      const status = getPlayerStatus(p.id);
      return status !== "injured" && status !== "resting";
    });
    ...
    const fieldCandidates = positions
      ...
      .filter(({ pid }) => getPlayerStatus(pid) !== "resting");
    if (fieldCandidates.length === 0) return [];

    const sortedBench = benchPlayers
      .slice()
      .sort((a,b) => {
        const statusRank = { available:0, tired:1 };
        const aRank = statusRank[getPlayerStatus(a.id)] ?? 2;
        const bRank = statusRank[getPlayerStatus(b.id)] ?? 2;
        if(aRank!==bRank) return aRank-bRank;
        return getTotal(a.id) - getTotal(b.id);
      })
      .slice(0, 3);
    const sortedField = fieldCandidates
      .slice()
      .sort((a,b) => {
        const outPriority = status => status==="injured" ? 3 : status==="tired" ? 2 : status==="available" ? 1 : 0;
        const aPriority = outPriority(getPlayerStatus(a.pid));
        const bPriority = outPriority(getPlayerStatus(b.pid));
        if(aPriority!==bPriority) return bPriority-aPriority;
        return getTotal(b.pid) - getTotal(a.pid);
      })
      .slice(0, 4);

    const ranked = [];
    sortedBench.forEach(inPlayer=>{
      sortedField.forEach(outCandidate=>{
        const outTotal = getTotal(outCandidate.pid);
        const inTotal = getTotal(inPlayer.id);
        ranked.push({
          posIdx: outCandidate.pi,
          outId: outCandidate.pid,
          inId: inPlayer.id,
          score: outTotal - inTotal,
        });
      });
    });
    ...
    return ranked
      ...
      .filter(item=>{
        const key = `${item.posIdx}-${item.outId}-${item.inId}`;
        if(seen.has(key)) return false;
        seen.add(key);
        return true;
      })
      .slice(0, 3);
  }, [roster, onField, positions, assigned, getTotal, playerStatuses]);

  // Keep ref fresh so interval can call it without stale closures
  useEffect(()=>{ computeRef.current = computeBestSwaps; }, [computeBestSwaps]);

  // Save to season on game over (once)
  useEffect(()=>{
    if(gameOver&&!savedGameRef.current&&roster.length>0&&gameTime>0){
      savedGameRef.current=true;
      const record={ date: new Date(`${gameDate}T12:00:00`).toISOString(), teamName, formation, opponent: gameOpponent.trim(), homeAway: gameHomeAway,
  ...
  },[gameOver, roster, gameTime, teamName, formation, numbers, getTotal, stats, gameDate, gameOpponent, gameHomeAway, teamScore, opponentScore, goalEvents]);
  ...
  const openPresentPlayers = returnPhase => {
    setPresentPlayersReturnPhase(returnPhase);
    setPhase("present-players");
  };
  const togglePresentPlayer = playerId => {
    const selected = presentPlayerIds.includes(playerId);
    if(selected && presentPlayersReturnPhase==="game") return;
    setPresentPlayerIds(prev=>selected?prev.filter(id=>id!==playerId):[...prev,playerId]);
    setPlayerStatuses(prev=>{
      ...
      return { ...prev, [playerId]:"available" };
    });
    setPresentPlayerSince(prev=>{
      if(selected){
        const next={...prev};
        delete next[playerId];
        return next;
      }
      return {...prev,[playerId]:presentPlayersReturnPhase==="game"?gameTime:0};
    });
  };
  const continueToLineup = ()=>{
    if(presentPlayerIds.length<playerCount){alert(`Select at least ${playerCount} present players.`);return;}
    setAssigned(prev=>prev.map(id=>presentPlayerIds.includes(id)?id:null));
    setPhase(presentPlayersReturnPhase);
  };
  const autoAssignLineup = ()=>{
    if(roster.length < positions.length){
      alert(`You need at least ${positions.length} available players to auto-fill this lineup.`);
      return;
    }
    const ids = roster.map(p=>p.id);
    const offset = ids.length===0 ? 0 : autoAssignCycleRef.current % ids.length;
    const rotated = ids.slice(offset).concat(ids.slice(0, offset));
    const remaining = [...rotated];
    const pickForRole = role => {
      const preferredIdx = remaining.findIndex(id=>{
        const pref = playerPrefs[id] || {};
        if(role==="GK") return pref.keeper || pref.preferredRole==="GK";
        return pref.preferredRole===role;
      });
      if(preferredIdx!==-1) return remaining.splice(preferredIdx,1)[0];

      const flexibleIdx = remaining.findIndex(id=>{
        const pref = playerPrefs[id] || {};
        return !pref.preferredRole && !pref.keeper;
      });
      if(flexibleIdx!==-1) return remaining.splice(flexibleIdx,1)[0];

      const nonKeeperIdx = remaining.findIndex(id=>!(playerPrefs[id]?.keeper));
      if(role!=="GK" && nonKeeperIdx!==-1) return remaining.splice(nonKeeperIdx,1)[0];

      return remaining.shift() ?? null;
    };
    setAssigned(positions.map(pos=>pickForRole(pos.role)));
    autoAssignCycleRef.current += 1;
    setPickerPos(null);
  };
  const handleHeaderBackToSetup = ()=>{
    ...
    if(phase==="game"){
      if(!confirm("Leave the current game and return to Team Settings?")) return;
      setIsRunning(false);
      setAlertActive(false);
      setShowStats(false);
      setShowSeason(false);
  ...
  const requestAlertPermission = async()=>{
    ...
    try {
      ...
      if(permission==="granted") {
        alert("Sub alerts enabled. Use Test Alert to check sound, vibration, and notifications.");
      } else if(permission==="denied") {
        alert("Notifications are blocked. Subby can still alert while the app is open, but lock-screen alerts may not work.");
      }
    } catch(e) {
  ...
  const testAlert = async()=>{
    await fireAlert();
    setAlertActive(true);
    setTimeout(()=>setAlertActive(false), 2200);
  };
  ...
  const openGoalSheet = ()=>{
    setGoalScorerId(roster[0]?.id ?? null);
    setGoalAssistId("");
    setShowGoalSheet(true);
  };
  const logGoal = ()=>{
    if(goalScorerId==null){
      alert("Choose the scorer first.");
      return;
    }
    ...
    setGoalEvents(prev=>[
      ...prev,
      {
        id: `${Date.now()}-${goalScorerId}`,
        time: gameTime,
        half,
        type: "goal",
        scorerId: goalScorerId,
        assistId: goalAssistId === "" ? null : Number(goalAssistId),
      }
    ]);
    closeGoalSheet();
  };
  const logOpponentGoal = ()=>{
  ...
  const startGame = ()=>{
    if(!lineupComplete){
      alert("Assign every position before starting the game.");
      return;
    }
    const init={};
    ...
    setPlayerStatuses(Object.fromEntries(roster.map(p=>[p.id, "available"])));
    setTeamScore(0); setOpponentScore(0); setGoalEvents([]); closeGoalSheet();
    setStats(init); setSC(subIntervalMins*60); scRef.current=subIntervalMins*60; setGameTime(0); setSubLog([]); setSubQueue([]);
    setAlertActive(false); setAutoSubSuggestions([]); setIsRunning(true); setTab("field"); setShowStats(false); setShowSeason(false);
    savedGameRef.current=false; setPhase("game");
  };
  const startSecondHalf = ()=>{ const newSC=subIntervalMins*60; setSC(newSC); scRef.current=newSC; setAlertActive(false); setAutoSubSuggestions([]); setIsRunning(true); };
  const resetToSetup = ()=>{
    setPhase("home"); setGameTime(0); setIsRunning(false); setStats({}); setSubLog([]); setSubQueue([]);
    setAlertActive(false); setAutoSubSuggestions([]); setShowStats(false); setShowSeason(false);
    setPlayerStatuses({});
    setTeamScore(0); setOpponentScore(0); setGoalEvents([]); closeGoalSheet();
    setAssigned(Array(buildPositions(formation).length).fill(null));
    autoAssignCycleRef.current = 0;
    setPickerPos(null);
    setGameDate(new Date().toISOString().slice(0,10));
    setGameOpponent("");
    setGameHomeAway("home");
    setPresentPlayerIds(masterRoster.map(p=>p.id));
    setPresentPlayerSince({});
    setPresentPlayersReturnPhase("field");
  };

  useEffect(()=>{
    if(!showCoachMenu) return;
    const handleOutside = e => {
      if(coachMenuRef.current && !coachMenuRef.current.contains(e.target)) {
        setShowCoachMenu(false);
      }
    };
  ...
  useEffect(()=>{
    const wasSaving = prevCloudSavingRef.current;
    if(cloudSaving){
      setSaveToast("Saving team...");
    } else if(wasSaving && lastSaved){
      setSaveToast("Team saved");
      clearTimeout(saveToastTimerRef.current);
      saveToastTimerRef.current = setTimeout(()=>setSaveToast(""), 1800);
    }
    prevCloudSavingRef.current = cloudSaving;
  },[cloudSaving, lastSaved]);
  ...
  if(!user) return <LandingPage sbReady={sbReady} onLogin={()=>{}}/>;

  // ── MAIN APP (logged in) ─────────────────────────────────────
  return (
    <div style={{fontFamily:"'DM Sans',system-ui,sans-serif",background:"linear-gradient(180deg,#0b1015 0%, #0d1416 38%, #0d1117 100%)",minHeight:"100vh",color:"#e6edf3",maxWidth:480,margin:"0 auto",position:"relative"}}>
      <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:0,overflow:"hidden"}}>
        <div style={{position:"absolute",inset:0,background:"radial-gradient(circle at top right, rgba(74,222,128,0.12), transparent 34%), radial-gradient(circle at 20% 30%, rgba(167,139,250,0.09), transparent 28%)"}}/>
        <div style={{position:"absolute",left:"50%",top:"54%",transform:"translate(-50%,-50%)",width:"92vw",maxWidth:540,height:"140vw",maxHeight:820,border:"1px solid rgba(255,255,255,0.085)",borderRadius:28}}/>
        <div style={{position:"absolute",left:"50%",top:"54%",transform:"translate(-50%,-50%)",width:"46vw",maxWidth:260,height:"46vw",maxHeight:260,border:"1px solid rgba(255,255,255,0.08)",borderRadius:"50%"}}/>
        <div style={{position:"absolute",left:"50%",top:"54%",transform:"translateX(-50%)",width:"92vw",maxWidth:540,height:1,background:"rgba(255,255,255,0.075)"}}/>
        <div style={{position:"absolute",left:"50%",top:"18%",transform:"translateX(-50%)",width:"58vw",maxWidth:320,height:"20vw",maxHeight:110,border:"1px solid rgba(255,255,255,0.07)",borderBottom:"none",borderRadius:"18px 18px 0 0"}}/>
        <div style={{position:"absolute",left:"50%",bottom:"10%",transform:"translateX(-50%)",width:"58vw",maxWidth:320,height:"20vw",maxHeight:110,border:"1px solid rgba(255,255,255,0.07)",borderTop:"none",borderRadius:"0 0 18px 18px"}}/>
      </div>

      {/* HEADER */}
      <div style={{background:"rgba(13,17,23,0.9)",backdropFilter:"blur(10px)",borderBottom:"1px solid #21262d",padding:"10px 16px 8px",position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
          <div style={{minWidth:0,flex:1}}>
            <div style={{fontSize:phase==="game"?9:10,letterSpacing:2.4,textTransform:"uppercase",color:"#a78bfa",fontWeight:800,marginBottom:2}}>Subby</div>
            <div style={{fontSize:phase==="game"?16:18,fontWeight:800,letterSpacing:-0.25,lineHeight:1.05,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
              {teamName||"My Team"}
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
            {phase==="game"&&(
              <div style={{textAlign:"right",minWidth:84}}>
                ...
                <div style={{fontSize:42,fontFamily:"'Bebas Neue',sans-serif",letterSpacing:2,lineHeight:0.9,color:gameOver?"#4ade80":"white"}}>{fmt(halfDisplay)}</div>
              </div>
            )}
            <div ref={coachMenuRef} style={{position:"relative"}}>
              <button
                onClick={()=>setShowCoachMenu(v=>!v)}
                style={{width:40,height:40,borderRadius:"50%",border:"1px solid rgba(74,222,128,0.45)",background:"linear-gradient(135deg,#4ade80,#16a34a)",color:"#0d1117",fontSize:13,fontWeight:800,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 6px 18px rgba(22,163,74,0.22)"}}>
                {coachInitials}
              </button>
              {showCoachMenu&&(
                <div style={{position:"absolute",right:0,top:"calc(100% + 10px)",width:228,background:"#161b22",border:"1px solid #30363d",borderRadius:14,boxShadow:"0 18px 40px rgba(0,0,0,0.45)",padding:10,zIndex:160}}>
                  <div style={{padding:"6px 8px 10px",borderBottom:"1px solid #21262d",marginBottom:8}}>
                    <div style={{fontSize:13,fontWeight:800,color:"#e6edf3",marginBottom:3}}>{coachName}</div>
                    <div style={{fontSize:11,color:"#8b949e",lineHeight:1.4,wordBreak:"break-word"}}>{user.email}</div>
                  </div>
                  {(phase==="field"||phase==="game")&&(
                    <button
                      onClick={handleHeaderBackToSetup}
                      style={{width:"100%",background:"transparent",border:"none",color:"#e6edf3",padding:"10px 8px",textAlign:"left",fontSize:13,fontWeight:700,cursor:"pointer",borderRadius:8}}>
                      ← Back to Setup
                    </button>
                  )}
                  <button
                    onClick={()=>{setShowCoachMenu(false);handleLogout();}}
                    style={{width:"100%",background:"transparent",border:"none",color:"#ef4444",padding:"10px 8px",textAlign:"left",fontSize:13,fontWeight:700,cursor:"pointer",borderRadius:8}}>
                    Log Out
                  </button>
                </div>
              )}
      ...
      {saveToast&&(
        <div style={{position:"fixed",top:70,left:"50%",transform:"translateX(-50%)",width:"calc(100% - 32px)",maxWidth:300,zIndex:140,pointerEvents:"none"}}>
          <div style={{background:"rgba(22,27,34,0.94)",border:`1px solid ${cloudSaving?"rgba(245,158,11,0.45)":"rgba(74,222,128,0.3)"}`,borderRadius:999,padding:"8px 12px",fontSize:12,fontWeight:700,color:cloudSaving?"#f59e0b":"#8b949e",textAlign:"center",boxShadow:"0 12px 28px rgba(0,0,0,0.28)"}}>
            {saveToast}
          </div>
        </div>
      )}
      ...
      {/* ══ HOME ══ */}
      {phase==="home"&&!showSeason&&(
        <div style={{padding:"20px 20px 48px",animation:"pageIn .3s ease"}}>
          <div style={{position:"relative",background:"radial-gradient(circle at top right, rgba(167,139,250,0.2), transparent 38%), linear-gradient(160deg,#1a1428 0%, #141922 48%, #161b22 100%)",border:"1px solid rgba(167,139,250,0.16)",borderRadius:22,padding:"18px 18px 16px",marginBottom:18,overflow:"hidden",boxShadow:"0 18px 36px rgba(0,0,0,0.22)"}}>
            <div style={{position:"absolute",right:-18,top:18,width:110,height:110,borderRadius:"50%",background:"radial-gradient(circle, rgba(74,222,128,0.14), transparent 72%)"}}/>
            <div style={{position:"absolute",inset:0,background:"linear-gradient(transparent 72%, rgba(255,255,255,0.02) 72%, rgba(255,255,255,0.02) 74%, transparent 74%, transparent 86%, rgba(255,255,255,0.02) 86%, rgba(255,255,255,0.02) 88%, transparent 88%)",pointerEvents:"none"}}/>
            <div style={{position:"relative"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,marginBottom:16}}>
                <div>
                  <div style={{fontSize:11,color:"#a78bfa",textTransform:"uppercase",letterSpacing:2,fontWeight:800,marginBottom:6}}>Game Day Launchpad</div>
                  <div style={{fontSize:28,fontWeight:800,lineHeight:1.02,letterSpacing:-0.6,marginBottom:8}}>{teamName||"Your Team"}</div>
                  <div style={{fontSize:13,color:"#9ca3af",lineHeight:1.55,maxWidth:230}}>
                    Ready to manage the sideline with saved players, saved formations, and live sub decisions.
                  </div>
                </div>
                <div style={{width:64,height:64,borderRadius:18,background:"rgba(74,222,128,0.1)",border:"1px solid rgba(74,222,128,0.24)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,boxShadow:"inset 0 0 24px rgba(74,222,128,0.08)"}}>
                  ⚽
                </div>
              </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
                {[
                  {label:"Players",value:masterRoster.length},
                  {label:"Format",value:`${playerCount}v${playerCount}`},
                  {label:"Formations",value:catalog.length},
                ].map(item=>(
                  <div key={item.label} style={{background:"rgba(13,17,23,0.48)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:14,padding:"12px 10px"}}>
                    <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.4,fontWeight:700,marginBottom:6}}>{item.label}</div>
                    <div style={{fontSize:18,fontWeight:800,color:"#e6edf3"}}>{item.value}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <button
            onClick={()=>{ if(masterRoster.length<playerCount){ setPhase("setup"); return; } setGameDate(new Date().toISOString().slice(0,10)); setGameOpponent(""); setGameHomeAway("home"); setPresentPlayerIds(masterRoster.map(p=>p.id)); setPresentPlayerSince(Object.fromEntries(masterRoster.map(p=>[p.id,0]))); setPresentPlayersReturnPhase("field"); setAssigned(Array(buildPositions(formation).length).fill(null)); setPhase("new-game"); }}
            style={{width:"100%",padding:"18px 20px",background:"linear-gradient(135deg,#4ade80,#34d399)",border:"none",borderRadius:18,color:"#0d1117",fontSize:19,fontWeight:800,cursor:"pointer",marginBottom:14,boxShadow:"0 18px 30px rgba(52,211,153,0.24)"}}>
            New Game
          </button>

          {masterRoster.length<playerCount&&(
            <div style={{fontSize:12,color:"#f59e0b",textAlign:"center",marginBottom:14}}>
              Add at least {playerCount} saved players in Team Settings before starting a game.
            </div>
          )}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
            <button
              ...
              style={{background:"#1b222c",border:"1px solid #2a3340",borderRadius:16,color:"#e6edf3",padding:"16px 14px",textAlign:"left",cursor:"pointer"}}>
              <div style={{fontSize:15,fontWeight:800,marginBottom:4}}>Team Settings</div>
              <div style={{fontSize:12,color:"#8b949e",lineHeight:1.45}}>Roster, formations, and default game setup</div>
            </button>

            <button
              onClick={()=>setShowSeason(true)}
              style={{background:"#151b24",border:"1px solid #2a3340",borderRadius:16,color:"#e6edf3",padding:"16px 14px",textAlign:"left",cursor:"pointer"}}>
              <div style={{fontSize:15,fontWeight:800,marginBottom:4}}>Season Stats</div>
              <div style={{fontSize:12,color:"#8b949e",lineHeight:1.45}}>{seasonGames.length>0?`${seasonGames.length} saved match reports`:"Minutes, history, and shareable reports"}</div>
            </button>
          </div>

          {latestGame&&(
            <div style={{background:"linear-gradient(180deg,#151b24,#11161d)",border:"1px solid #21262d",borderRadius:16,padding:"14px 16px"}}>
              <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:8}}>Last Match</div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
                <div>
                  <div style={{fontSize:16,fontWeight:800,marginBottom:4}}>{latestGameLabel}</div>
                  <div style={{fontSize:12,color:"#8b949e"}}>{new Date(latestGame.date).toLocaleDateString("en-US",{month:"short",day:"numeric"})} · {latestGame.formation}</div>
                </div>
                <button onClick={()=>setShowSeason(true)} style={{background:"transparent",border:"1px solid #30363d",borderRadius:999,color:"#8b949e",padding:"8px 12px",fontSize:12,fontWeight:700,cursor:"pointer"}}>
                  View
                </button>
              </div>
            </div>
      ...
      {phase==="setup"&&!showSeason&&(
        <div style={{padding:"20px 20px 120px",animation:"pageIn .3s ease"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
            <div>
              <div style={{fontSize:18,fontWeight:800}}>Team Settings</div>
              <div style={{fontSize:12,color:"#8b949e",marginTop:4}}>Saved to your account for every game day</div>
            </div>
            <button onClick={()=>setPhase("home")} style={{background:"#21262d",border:"1px solid #30363d",color:"#8b949e",borderRadius:8,padding:"7px 14px",fontSize:12,fontWeight:600,cursor:"pointer"}}>← Home</button>
          </div>
          ...
          <div style={{marginBottom:22,background:"#161b22",border:"1px solid #21262d",borderRadius:14,overflow:"hidden"}}>
            <button
              onClick={()=>setShowFormationSettings(v=>!v)}
              style={{width:"100%",background:"transparent",border:"none",padding:"14px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",color:"#e6edf3",cursor:"pointer",textAlign:"left"}}>
              <div>
                <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:5}}>Saved Formations</div>
                <div style={{fontSize:13,color:"#8b949e"}}>{catalog.length} available for {outfieldCount} outfield + GK</div>
              </div>
              <div style={{fontSize:16,color:"#8b949e"}}>{showFormationSettings?"−":"+"}</div>
            </button>
            {showFormationSettings&&(
          ...
          <div style={{marginBottom:22,background:"#161b22",border:"1px solid #21262d",borderRadius:14,overflow:"hidden"}}>
            <button
              ...
              style={{width:"100%",background:"transparent",border:"none",padding:"14px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",color:"#e6edf3",cursor:"pointer",textAlign:"left"}}>
              <div>
                <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:5}}>Roster</div>
                <div style={{fontSize:13,color:"#8b949e"}}>{masterRoster.length} saved players</div>
              </div>
              <div style={{fontSize:16,color:"#8b949e"}}>{showRosterSettings?"−":"+"}</div>
            </button>
            {showRosterSettings&&(
              <div style={{padding:"0 16px 16px",display:"grid",gap:7}}>
                {names.map((n,i)=>(
                  <div key={i} style={{background:"#0d1117",border:"1px solid #30363d",borderRadius:10,padding:9}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 56px",gap:7}}>
                      <input value={n} onChange={e=>setNames(prev=>{const nx=[...prev];nx[i]=e.target.value;return nx;})}
                        placeholder={`Player ${i+1}`}
                        style={{background:"#11161d",border:"1px solid #30363d",borderRadius:8,padding:"9px 12px",color:"#e6edf3",fontSize:13}}/>
                      <input value={numbers[i]} onChange={e=>setNumbers(prev=>{const nx=[...prev];nx[i]=e.target.value;return nx;})}
                        placeholder="#"
                        style={{background:"#11161d",border:"1px solid #30363d",borderRadius:8,padding:"9px 8px",color:"#e6edf3",fontSize:13,textAlign:"center"}}/>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:8,marginTop:8,alignItems:"center"}}>
                      <select value={playerPrefs[i]?.preferredRole || ""} onChange={e=>updatePlayerPref(i,{ preferredRole:e.target.value })}
                        style={{background:"#11161d",border:"1px solid #30363d",borderRadius:8,padding:"9px 12px",color:"#e6edf3",fontSize:12}}>
                        <option value="">No preferred position</option>
                        {ROLE_OPTIONS.map(role=><option key={role} value={role}>{role}</option>)}
                      </select>
                      <button
                        onClick={()=>updatePlayerPref(i,{ keeper: !playerPrefs[i]?.keeper })}
                        style={{background:playerPrefs[i]?.keeper?"rgba(245,158,11,0.18)":"#11161d",border:`1px solid ${playerPrefs[i]?.keeper?"rgba(245,158,11,0.5)":"#30363d"}`,borderRadius:8,padding:"9px 12px",color:playerPrefs[i]?.keeper?"#fbbf24":"#8b949e",fontSize:12,fontWeight:800,cursor:"pointer",whiteSpace:"nowrap"}}>
                        GK
                      </button>
                    </div>
                  </div>
                ))}
                <div style={{fontSize:11,color:"#6b7280",lineHeight:1.45,padding:"4px 2px 0"}}>
                  Preferred position and GK flags help Auto Roster place players more intelligently on game day.
                </div>
              </div>
            )}
          </div>
      ...
      {phase==="new-game"&&!showSeason&&(
        <div style={{padding:"20px 20px 48px",animation:"pageIn .3s ease"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
            <div>
              <div style={{fontSize:18,fontWeight:800}}>New Game</div>
              <div style={{fontSize:12,color:"#8b949e",marginTop:4}}>Add the key details before you set your lineup</div>
            </div>
            <button onClick={()=>setPhase("home")} style={{background:"#21262d",border:"1px solid #30363d",color:"#8b949e",borderRadius:8,padding:"7px 14px",fontSize:12,fontWeight:600,cursor:"pointer"}}>← Home</button>
          </div>

          <Section label="Game Date">
            <input value={gameDate} onChange={e=>setGameDate(e.target.value)} type="date"
              style={{width:"100%",background:"#161b22",border:"1px solid #30363d",borderRadius:10,padding:"11px 14px",color:"#e6edf3",fontSize:15}}/>
          </Section>

          <Section label="Home or Away">
            <div style={{display:"flex",gap:8}}>
              {[
                ["home","Home"],
                ["away","Away"],
              ].map(([value,label])=>(
                <Chip key={value} active={gameHomeAway===value} onClick={()=>setGameHomeAway(value)} flex>{label}</Chip>
              ))}
            </div>
          </Section>

          <Section label="Opponent">
            <input value={gameOpponent} onChange={e=>setGameOpponent(e.target.value)} placeholder="e.g. FC Stars"
              style={{width:"100%",background:"#161b22",border:"1px solid #30363d",borderRadius:10,padding:"11px 14px",color:"#e6edf3",fontSize:15}}/>
          </Section>

          <button onClick={()=>setPhase("present-players")}
            style={{width:"100%",padding:15,background:"#4ade80",border:"none",borderRadius:12,color:"#0d1117",fontSize:16,fontWeight:800,cursor:"pointer"}}>
            Continue
          </button>
        </div>
      )}

      {phase==="present-players"&&!showSeason&&(
        <div style={{padding:"20px 20px 48px",animation:"pageIn .3s ease"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
            <div>
              <div style={{fontSize:18,fontWeight:800}}>Present Players</div>
              <div style={{fontSize:12,color:"#8b949e",marginTop:4}}>Select who is here today so the bench and subs stay accurate</div>
            </div>
            <button onClick={()=>setPhase(presentPlayersReturnPhase==="game"?"game":presentPlayersReturnPhase==="field"?"field":"new-game")} style={{background:"#21262d",border:"1px solid #30363d",color:"#8b949e",borderRadius:8,padding:"7px 14px",fontSize:12,fontWeight:600,cursor:"pointer"}}>{presentPlayersReturnPhase==="game"?"← Game":presentPlayersReturnPhase==="field"?"← Lineup":"← Details"}</button>
          </div>

          <div style={{background:"#161b22",border:"1px solid #21262d",borderRadius:14,padding:"14px 16px",marginBottom:16}}>
            <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:6}}>Today&apos;s Squad</div>
            <div style={{fontSize:14,fontWeight:700}}>{presentPlayerIds.length} selected</div>
            <div style={{fontSize:12,color:"#8b949e",marginTop:4}}>You need at least {playerCount} players available to continue.</div>
          </div>

          <div style={{display:"grid",gap:8,marginBottom:16}}>
            {masterRoster.map(p=>{
              const selected=presentPlayerIds.includes(p.id);
              return (
                <button key={p.id} onClick={()=>togglePresentPlayer(p.id)}
                  style={{background:selected?"#1a2e1a":"#161b22",border:`1px solid ${selected?"#4ade80":"#21262d"}`,borderRadius:12,padding:"13px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",color:"#e6edf3",cursor:"pointer"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div style={{width:18,height:18,borderRadius:"50%",background:selected?"#4ade80":"#30363d",display:"flex",alignItems:"center",justifyContent:"center",color:selected?"#0d1117":"transparent",fontSize:12,fontWeight:800,flexShrink:0}}>
                      ✓
                    </div>
                    <div style={{textAlign:"left"}}>
                      <div style={{fontSize:15,fontWeight:700}}>{p.name}</div>
                      {numbers[p.id]&&<div style={{fontSize:11,color:"#8b949e",marginTop:2}}>#{numbers[p.id]}</div>}
                    </div>
                  </div>
                  <div style={{fontSize:12,color:selected?"#4ade80":"#6b7280",fontWeight:700}}>{selected?(presentPlayersReturnPhase==="game"?"Available":"Present"):"Out"}</div>
                </button>
              );
            })}
          </div>

          <button onClick={continueToLineup}
            style={{width:"100%",padding:15,background:"#4ade80",border:"none",borderRadius:12,color:"#0d1117",fontSize:16,fontWeight:800,cursor:"pointer"}}>
            {presentPlayersReturnPhase==="game"?"Return to Game":"Continue to Lineup"}
          </button>
        </div>
      )}

      {phase==="setup"&&!showSeason&&(
        <div style={{position:"sticky",bottom:0,zIndex:90,padding:"12px 20px calc(12px + env(safe-area-inset-bottom, 0px))",background:"linear-gradient(180deg,rgba(13,17,23,0),#0d1117 28%,#0d1117 100%)"}}>
          <button onClick={()=>{ if(masterRoster.length<playerCount){alert(`Add at least ${playerCount} player names.`);return;} setPhase("home"); }}
            style={{width:"100%",padding:15,background:"#4ade80",border:"none",borderRadius:12,color:"#0d1117",fontSize:16,fontWeight:800,cursor:"pointer",boxShadow:"0 10px 28px rgba(0,0,0,0.35)"}}>
            Save Team Settings
          </button>
        </div>
      )}
        ...
        <div style={{animation:"pageIn .3s ease"}}>
          ...
          {alertActive&&(
            <div style={{background:"linear-gradient(90deg,#d97706,#dc2626)",padding:"14px 16px",animation:"alertPulse 1s ease-in-out infinite"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom: autoSubSuggestions.length ? 10 : 0}}>
                <div style={{fontSize:16,fontWeight:700}}>
                  {autoSub ? "🤖 Auto Sub Ready" : "🔔 Time to Sub!"}
                </div>
                <button onClick={()=>{setAlertActive(false);const newSC=subIntervalMins*60;setSC(newSC);scRef.current=newSC;setAutoSubSuggestions([]);}}
              ...
              {/* Auto-sub suggestions */}
              {autoSubSuggestions.length>0&&(
                <div style={{display:"grid",gap:8}}>
                  {autoSubSuggestions.map((suggestion, idx)=>{
                    const inP  = roster.find(p=>p.id===suggestion.inId);
                    const outP = roster.find(p=>p.id===suggestion.outId);
                    const pos  = positions[suggestion.posIdx];
                    return (
                      <div key={`${suggestion.posIdx}-${suggestion.outId}-${suggestion.inId}`} style={{background:"rgba(0,0,0,0.25)",borderRadius:12,padding:"10px 12px",display:"flex",alignItems:"center",gap:10}}>
                        <div style={{flex:1}}>
                          <div style={{fontSize:12,color:"rgba(255,255,255,0.7)",marginBottom:4}}>
                            {idx===0 ? "Top suggestion" : `Option ${idx+1}`} · {pos?.role}
                          </div>
                          <div style={{display:"flex",alignItems:"center",gap:8}}>
                            <div style={{textAlign:"center"}}>
                              <div style={{fontSize:14,fontWeight:800,color:"#fca5a5"}}>{outP?.name||"?"}</div>
                              <div style={{fontSize:10,color:"rgba(255,255,255,0.6)"}}>{fmtMin(getTotal(suggestion.outId))} played</div>
                            </div>
                            <div style={{fontSize:18,color:"rgba(255,255,255,0.5)"}}>→</div>
                            <div style={{textAlign:"center"}}>
                              <div style={{fontSize:14,fontWeight:800,color:"#86efac"}}>{inP?.name||"?"}</div>
                              <div style={{fontSize:10,color:"rgba(255,255,255,0.6)"}}>{fmtMin(getTotal(suggestion.inId))} played</div>
                            </div>
                          </div>
                        </div>
                        <button onClick={()=>handleAssign(suggestion.posIdx,suggestion.inId)}
                  ...
                  <button onClick={()=>{setAutoSubSuggestions([]);setTab("field");}}
                    style={{background:"rgba(255,255,255,0.15)",border:"none",color:"white",borderRadius:10,padding:"8px 14px",fontSize:11,fontWeight:700,cursor:"pointer"}}>
                    Choose Different
                  </button>
                </div>
              )}
              ...
              {!autoSubSuggestions.length&&subQueue.length>0&&(
          ...
          <div style={{padding:"12px 16px",borderBottom:"1px solid #21262d",display:"flex",alignItems:"center",gap:10}}>
            {phase==="field"?(
              <button onClick={startGame} disabled={!lineupComplete} style={{flex:1,padding:"11px 0",background:lineupComplete?"#4ade80":"#21262d",border:"none",borderRadius:10,color:lineupComplete?"#0d1117":"#4b5563",fontSize:15,fontWeight:700,cursor:lineupComplete?"pointer":"not-allowed"}}>{lineupComplete?"🏁 Start Game":"Assign Full Lineup"}</button>
            ):atHalftime?(
            ...
            {phase==="game"&&(
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                <button onClick={()=>openPresentPlayers("game")} style={{background:"#21262d",border:"1px solid #30363d",color:"#e6edf3",borderRadius:8,padding:"8px 10px",fontSize:13,cursor:"pointer"}}>👥</button>
          ...
          {phase==="game"&&(
            <div style={{padding:"12px 16px",borderBottom:"1px solid #21262d"}}>
              <div style={{background:"#161b22",border:"1px solid #21262d",borderRadius:14,padding:"14px 14px 12px"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:4}}>Scoreboard</div>
                    <div style={{fontSize:13,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{teamName || "Your Team"}</div>
                  </div>
                  <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:44,letterSpacing:2,lineHeight:0.9,color:"#4ade80",flexShrink:0}}>
                    {teamScore}<span style={{color:"#6b7280",margin:"0 6px"}}>-</span><span style={{color:"#e6edf3"}}>{opponentScore}</span>
                  </div>
                  <div style={{flex:1,minWidth:0,textAlign:"right"}}>
                    <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:4}}>Opponent</div>
                    <div style={{fontSize:13,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{opponentLabel}</div>
                  </div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:12}}>
                  <button onClick={openGoalSheet}
                    style={{padding:"10px 12px",background:"#4ade80",border:"none",borderRadius:10,color:"#0d1117",fontSize:13,fontWeight:800,cursor:"pointer"}}>
                    + Goal
                  </button>
                  <button onClick={logOpponentGoal}
                    style={{padding:"10px 12px",background:"#21262d",border:"1px solid #30363d",borderRadius:10,color:"#f87171",fontSize:13,fontWeight:800,cursor:"pointer"}}>
                    Opp Goal
                  </button>
                </div>
                {goalEvents.length>0&&(
                  <div style={{marginTop:10,fontSize:11,color:"#8b949e"}}>
                    Last: {goalEvents[goalEvents.length-1].type==="goal"
                      ? `${roster.find(p=>p.id===goalEvents[goalEvents.length-1].scorerId)?.name || "Goal"} at ${fmt(goalEvents[goalEvents.length-1].time || 0)}`
                      : `Opponent goal at ${fmt(goalEvents[goalEvents.length-1].time || 0)}`
                    }
                  </div>
                )}
              </div>
          ...
          {tab==="field"&&(
            <div style={{padding:"14px 16px 32px"}}>
              {phase==="field"&&(
                <div style={{background:"#161b22",border:"1px solid #21262d",borderRadius:14,padding:"14px 14px 16px",marginBottom:14}}>
                  <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:12}}>Game Options</div>
                  <div style={{background:"#0d1117",border:"1px solid #30363d",borderRadius:12,padding:"14px 16px",marginBottom:14}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12}}>
                      <div>
                        <div style={{fontSize:14,fontWeight:700,color:"#e6edf3"}}>Alert Readiness</div>
                        <div style={{fontSize:11,color:"#8b949e",marginTop:4,lineHeight:1.45}}>
                          Subby already plays a loud alert in the app. Turn on notifications for the best chance of feeling it when your phone is tucked away.
                        </div>
                      </div>
                      <div style={{fontSize:11,fontWeight:800,color:notificationPermission==="granted"?"#4ade80":notificationPermission==="denied"?"#ef4444":"#f59e0b",background:notificationPermission==="granted"?"rgba(74,222,128,0.12)":notificationPermission==="denied"?"rgba(239,68,68,0.12)":"rgba(245,158,11,0.12)",padding:"5px 8px",borderRadius:999,whiteSpace:"nowrap"}}>
                        {notificationPermission==="granted"?"Alerts On":notificationPermission==="denied"?"Alerts Blocked":notificationPermission==="unsupported"?"Not Supported":"Needs Setup"}
                      </div>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:12}}>
                      <button
                        ...
                        style={{padding:"10px 12px",background:notificationPermission==="granted"?"#1a2e1a":"#21262d",border:`1px solid ${notificationPermission==="granted"?"#4ade80":"#30363d"}`,borderRadius:10,color:notificationPermission==="granted"?"#4ade80":"#e6edf3",fontSize:12,fontWeight:700,cursor:notificationPermission==="unsupported" || notificationPermission==="granted"?"default":"pointer",opacity:notificationPermission==="unsupported"?0.55:1}}>
                        {notificationPermission==="granted"?"Notifications Ready":"Request Alerts"}
                      </button>
                      <button
                        onClick={testAlert}
                        style={{padding:"10px 12px",background:"#21262d",border:"1px solid #30363d",borderRadius:10,color:"#e6edf3",fontSize:12,fontWeight:700,cursor:"pointer"}}>
                        Test Alert
                      </button>
                    </div>
                    <div style={{fontSize:10,color:"#6b7280",marginTop:10,lineHeight:1.45}}>
                      Best results: keep the screen awake during games and allow notifications on this device.
                    </div>
                  </div>
                  <div style={{marginBottom:14}}>
                    <div style={{fontSize:12,color:"#8b949e",fontWeight:700,marginBottom:8}}>Sub alert every (minutes)</div>
                    <div style={{display:"flex",gap:8}}>
                      {[5,6,7,8,10,12].map(m=><Chip key={m} active={subIntervalMins===m} onClick={()=>setSIM(m)} flex>{m}</Chip>)}
                    </div>
                  </div>
                  <button onClick={()=>openPresentPlayers("field")}
                    style={{width:"100%",padding:"10px 12px",background:"#21262d",border:"1px solid #30363d",borderRadius:10,color:"#e6edf3",fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:14}}>
                    Edit Present Players ({presentPlayerIds.length})
                  </button>
                  <div style={{background:"#0d1117",border:`1px solid ${autoSub?"#4ade80":"#30363d"}`,borderRadius:12,padding:"14px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",transition:"border-color .2s"}}
                    onClick={()=>setAutoSub(v=>!v)}>
                    <div>
                      <div style={{fontSize:14,fontWeight:700,color:autoSub?"#4ade80":"#e6edf3"}}>🤖 Smart Auto Sub</div>
                      <div style={{fontSize:11,color:"#8b949e",marginTop:3,lineHeight:1.4}}>Set this for today&apos;s game. You can still choose a different player manually during sub alerts.</div>
                    </div>
              ...
              <div style={{marginBottom:14}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:8}}>
                  <div style={{fontSize:10,color:"#8b949e",textTransform:"uppercase",letterSpacing:1.5,fontWeight:700}}>Formation · {playerCount} players</div>
                  {phase==="field"&&(
                    <button
                      onClick={autoAssignLineup}
                      style={{background:"#7c3aed22",border:"1px solid #7c3aed",color:"#c4b5fd",borderRadius:9,padding:"7px 12px",fontSize:12,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>
                      Auto Roster
                    </button>
                  )}
                </div>
                  ...
                  {bench.length===0
                    ...
                    : <div style={{display:"grid",gap:8}}>
                        {bench.map(p=>{
                          ...
                          const status = getPlayerStatus(p.id);
                          const statusMeta = PLAYER_STATUSES[status] || PLAYER_STATUSES.available;
                          return (
                            <div key={p.id} style={{background:"#161b22",border:"1px solid #21262d",borderRadius:10,padding:"10px 13px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                              <div style={{display:"flex",alignItems:"center",gap:10}}>
                                ...
                                <div>
                                  ...
                                  {phase==="game"&&(
                                    <button
                                      onClick={()=>setPlayerStatus(p.id, status==="available"?"tired":status==="tired"?"resting":status==="resting"?"injured":"available")}
                                      style={{marginTop:5,background:`${statusMeta.color}18`,border:`1px solid ${statusMeta.color}44`,borderRadius:999,padding:"2px 8px",fontSize:10,color:statusMeta.color,fontWeight:800,cursor:"pointer"}}>
                                      {statusMeta.label}
                                    </button>
                                  )}
      ...
      {showGoalSheet&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.72)",zIndex:320,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={closeGoalSheet}>
          <div style={{background:"#161b22",borderRadius:"22px 22px 0 0",padding:"20px 20px 44px",width:"100%",maxWidth:480,border:"1px solid #30363d",animation:"sheetUp .22s ease"}} onClick={e=>e.stopPropagation()}>
            <div style={{width:36,height:4,background:"#30363d",borderRadius:2,margin:"0 auto 18px"}}/>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div>
                <div style={{fontSize:18,fontWeight:800}}>Log Goal</div>
                <div style={{fontSize:12,color:"#8b949e",marginTop:4}}>Add scorer and optional assist for this game moment</div>
              </div>
              <button onClick={closeGoalSheet} style={{background:"#21262d",border:"1px solid #30363d",color:"#8b949e",borderRadius:8,padding:"6px 12px",fontSize:12,fontWeight:700,cursor:"pointer"}}>Close</button>
            </div>

            <Section label="Scorer">
              <div style={{display:"grid",gap:8,maxHeight:260,overflowY:"auto"}}>
                {roster.map(p=>(
                  <button key={p.id} onClick={()=>setGoalScorerId(p.id)}
                    style={{background:goalScorerId===p.id?"#1f3a2a":"#21262d",border:`1px solid ${goalScorerId===p.id?"#4ade80":"#30363d"}`,borderRadius:10,padding:"11px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",color:"#e6edf3",cursor:"pointer",fontSize:13,fontWeight:700}}>
                    <span>{p.name}{numbers[p.id]?<span style={{color:"#8b949e",fontSize:11}}> #{numbers[p.id]}</span>:null}</span>
                    {goalScorerId===p.id?<span style={{color:"#4ade80"}}>✓</span>:null}
                  </button>
                ))}
              </div>
            </Section>

            <Section label="Assist (optional)">
              <select value={goalAssistId} onChange={e=>setGoalAssistId(e.target.value)}
                style={{width:"100%",background:"#161b22",border:"1px solid #30363d",borderRadius:10,padding:"11px 14px",color:"#e6edf3",fontSize:15}}>
                <option value="">No assist</option>
                {roster.filter(p=>p.id!==goalScorerId).map(p=>(
                  <option key={p.id} value={String(p.id)}>{p.name}{numbers[p.id]?` #${numbers[p.id]}`:""}</option>
                ))}
              </select>
            </Section>

            <button onClick={logGoal}
              style={{width:"100%",padding:14,background:"#4ade80",border:"none",borderRadius:12,color:"#0d1117",fontSize:15,fontWeight:800,cursor:"pointer"}}>
              Save Goal
            </button>
          </div>
        </div>
      ...
      {pickerPos!==null&&<PickerSheet posIdx={pickerPos} positions={positions} assigned={assigned} roster={roster} numbers={numbers} notes={notes}
        ...
        getPlayerStatus={getPlayerStatus} setPlayerStatus={setPlayerStatus}/>}
