...
export default function App() {
  ...
  return (
    <div style={{fontFamily:"'DM Sans',system-ui,sans-serif",background:"#0d1117",minHeight:"100vh",color:"#e6edf3",maxWidth:480,margin:"0 auto"}}>
      ...
      {/* ══ AUTH MODALS ══ */}
      {authView&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>setAuthView(null)}>
          <div style={{background:"#161b22",borderRadius:20,padding:28,width:"100%",maxWidth:400,border:"1px solid #30363d",animation:"sheetUp .22s ease"}} onClick={e=>e.stopPropagation()}>
            ...
            {authView==="setup"&&(
              <>
                <div style={{fontSize:20,fontWeight:800,marginBottom:6}}>Connect Supabase</div>
                <div style={{fontSize:12,color:"#8b949e",marginBottom:20,lineHeight:1.6}}>
                  Your team data is stored securely in your own Supabase project.<br/>
                  <strong style={{color:"#e6edf3"}}>Setup takes 2 minutes:</strong>
                </div>
                {[
                  ["1","Go to","supabase.com","https://supabase.com","and create a free project"],
                  ["2","In your project go to","SQL Editor","","and run this to create the teams table:"],
                ].map(([n,pre,link,href,post])=>(
                  <div key={n} style={{display:"flex",gap:10,marginBottom:12,alignItems:"flex-start"}}>
                    <div style={{width:22,height:22,borderRadius:"50%",background:"#7c3aed",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,color:"white",flexShrink:0,marginTop:1}}>{n}</div>
                    <div style={{fontSize:12,color:"#8b949e",lineHeight:1.5}}>{pre} {href?<a href={href} target="_blank" rel="noopener" style={{color:"#a78bfa"}}>{link}</a>:<strong style={{color:"#e6edf3"}}>{link}</strong>} {post}</div>
                  </div>
                ))}
                <pre style={{background:"#0d1117",border:"1px solid #30363d",borderRadius:8,padding:"10px 12px",fontSize:10,color:"#a78bfa",overflowX:"auto",marginBottom:16,lineHeight:1.5,whiteSpace:"pre-wrap"}}>
{`create table if not exists teams (
  ...
  notes jsonb,
  formation text,
  custom_formations jsonb,
  sub_interval int,
  auto_sub boolean,
  updated_at timestamptz default now()
);
alter table teams enable row level security;
create policy "Own team" on teams
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);`}
                </pre>
                <div style={{display:"flex",gap:10,marginBottom:8,alignItems:"flex-start"}}>
                  <div style={{width:22,height:22,borderRadius:"50%",background:"#7c3aed",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,color:"white",flexShrink:0,marginTop:1}}>3</div>
                  <div style={{fontSize:12,color:"#8b949e"}}>Go to <strong style={{color:"#e6edf3"}}>Project Settings → API</strong> and paste your URL and anon key below</div>
                </div>
                <input value={sbUrl} onChange={e=>setSbUrl(e.target.value)} placeholder="https://xxxx.supabase.co"
                  style={{width:"100%",background:"#0d1117",border:"1px solid #30363d",borderRadius:8,padding:"10px 12px",color:"#e6edf3",fontSize:13,marginBottom:8}}/>
                <input value={sbKey} onChange={e=>setSbKey(e.target.value)} placeholder="anon / public key"
                  style={{width:"100%",background:"#0d1117",border:"1px solid #30363d",borderRadius:8,padding:"10px 12px",color:"#e6edf3",fontSize:13,marginBottom:16}}/>
                <button onClick={saveCredentials} disabled={!sbUrl||!sbKey}
                  style={{width:"100%",padding:13,background:sbUrl&&sbKey?"#7c3aed":"#21262d",border:"none",borderRadius:12,color:sbUrl&&sbKey?"white":"#4b5563",fontSize:15,fontWeight:700,cursor:sbUrl&&sbKey?"pointer":"not-allowed"}}>
                  Save & Connect
                </button>
            ...
            {authView==="login"&&(
              <>
                ...
                <div style={{fontSize:12,color:"#8b949e",marginBottom:20}}>Log in to access your saved team</div>
                {authError&&<div style={{background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:8,padding:"9px 12px",fontSize:12,color:"#f87171",marginBottom:14}}>{authError}</div>}
                <input value={authEmail} onChange={e=>setAuthEmail(e.target.value)} placeholder="Email" type="email"
                  style={{width:"100%",background:"#0d1117",border:"1px solid #30363d",borderRadius:8,padding:"11px 12px",color:"#e6edf3",fontSize:14,marginBottom:10}}/>
                ...
                <button onClick={handleLogin} disabled={authLoading||!authEmail||!authPass}
                  style={{width:"100%",padding:13,background:authLoading||!authEmail||!authPass?"#21262d":"#4ade80",border:"none",borderRadius:12,color:authLoading||!authEmail||!authPass?"#4b5563":"#0d1117",fontSize:15,fontWeight:700,cursor:"pointer"}}>
            ...
            {authView==="signup"&&(
              <>
                ...
                <div style={{fontSize:12,color:"#8b949e",marginBottom:20}}>Save your team across all your devices</div>
                {authError&&<div style={{background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:8,padding:"9px 12px",fontSize:12,color:"#f87171",marginBottom:14}}>{authError}</div>}
                <input value={authName} onChange={e=>setAuthName(e.target.value)} placeholder="Your name (coach)"
                  style={{width:"100%",background:"#0d1117",border:"1px solid #30363d",borderRadius:8,padding:"11px 12px",color:"#e6edf3",fontSize:14,marginBottom:10}}/>
                <input value={authEmail} onChange={e=>setAuthEmail(e.target.value)} placeholder="Email" type="email"
                  style={{width:"100%",background:"#0d1117",border:"1px solid #30363d",borderRadius:8,padding:"11px 12px",color:"#e6edf3",fontSize:14,marginBottom:10}}/>
                <input value={authPass} onChange={e=>setAuthPass(e.target.value)} placeholder="Password (min 6 chars)" type="password"
